/*
let inputd82: string[] = ["be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb |fdgacbe cefdb cefbgd gcbe",
...
//console.log("inputd8 length should be 10: " + inputd82.length)
for(let i = 0; i < inputd82.length; i++)
{
    splitstrings2.length = 0
    shortstrings2.length = 0
    longstrings.length = 0
    shorterstrings2.length = 0
    longshortstrings.length = 0
    letterA = "-1"
    letterB = "-1"
    letterD = "-1"
    letterE = "-1"
    ...
    splitstrings2 = inputd82[i].split("|")
  //  console.log("Splitstrings length " + splitstrings2.length)
    for (let k = 0; k < splitstrings2.length; k++)
    {
        if (splitstrings2[k].length < 40)
        {
            shortstrings2.push(splitstrings2[k])
        }
        else
        {
            longstrings.push(splitstrings2[k])
        }
    }

  //  console.log("longstrings length is: " + longstrings.length)
    for(let p = 0; p < longstrings.length; p++)
    {   
      //  console.log("Long Strings: " + longstrings[p])
        longshortstrings = longstrings[p].split(" ")
        for (let q = 0; q < longshortstrings.length; q++)
        {
            
            if (longshortstrings[q].trim().length == 2)
            {        
                letterA = longshortstrings[q].trim().charAt(0);
                letterB = longshortstrings[q].trim().charAt(1);     
            }
        }
        for (let r = 0; r < longshortstrings.length; r++)
        {
            if (longshortstrings[r].trim().length == 3)
            {   var temp: string = longshortstrings[r]; 
                var temp2: string = ""
                var temp3: string = ""
                temp2 = temp.replace(letterA, '')
                temp3 = temp2.replace(letterB, '');
                letterD = temp3;     
            }
            if (longshortstrings[r].trim().length == 4)
            {        
                letterE = longshortstrings[r].trim().replace(letterA, '').replace(letterB, '').charAt(0);     
                letterF = longshortstrings[r].trim().replace(letterA, '').replace(letterB, '').charAt(1);  
            }    
        }  
   // console.log("Letters: A=" + letterA + " B=" + letterB + " D=" + letterD + " E=" + letterE + " F=" + letterF)
    }

    for(let m = 0; m < shortstrings2.length; m++)
    {
       // console.log("Short Strings: " + shortstrings2[m] + " Short Strings Length = " + shortstrings2.length);
        shorterstrings2 = shortstrings2[m].split(" ")
        for (let n = 0; n < shorterstrings2.length; n++)
        {
           // console.log("n=" + n + " " + shorterstrings2[n] + " length " + shorterstrings2[n].length + " fd=" + firstDigit + " sd=" + secondDigit + " td=" + thirdDigit + " 4d=" + fourthDigit)
           
            if (shorterstrings2[n].trim().length == 6 && !(shorterstrings2[n].includes(letterE) && shorterstrings2[n].includes(letterB)  && shorterstrings2[n].includes(letterF) && shorterstrings2[n].includes(letterA)))
            {        
                if(n == 1){firstDigit = 0}
                if(n == 2){secondDigit = 0} 
                if(n == 3){thirdDigit = 0}
                if(n == 4){fourthDigit = 0}     
            }
            if (shorterstrings2[n].trim().length == 2)
            {        
                if(n == 1){firstDigit = 1}
                if(n == 2){secondDigit = 1} 
                if(n == 3){thirdDigit = 1}
                if(n == 4){fourthDigit = 1}     
            }
            if (shorterstrings2[n].trim().length == 5 && !(shorterstrings2[n].includes(letterE) && shorterstrings2[n].includes(letterF)))
            {        
                if(n == 1){firstDigit = 2}
                if(n == 2){secondDigit = 2} 
                if(n == 3){thirdDigit = 2}
                if(n == 4){fourthDigit = 2}     
            }
            if (shorterstrings2[n].trim().length == 5 && shorterstrings2[n].includes(letterA) && shorterstrings2[n].includes(letterB))
            {        
                if(n == 1){firstDigit = 3}
                if(n == 2){secondDigit = 3} 
                if(n == 3){thirdDigit = 3}
                if(n == 4){fourthDigit = 3}     
            }
            if (shorterstrings2[n].trim().length == 4)
            {        
                if(n == 1){firstDigit = 4}
                if(n == 2){secondDigit = 4} 
                if(n == 3){thirdDigit = 4}
                if(n == 4){fourthDigit = 4}     
            }
            if (shorterstrings2[n].trim().length == 5 && shorterstrings2[n].includes(letterE) && shorterstrings2[n].includes(letterF))
            {        
                if(n == 1){firstDigit = 5}
                if(n == 2){secondDigit = 5} 
                if(n == 3){thirdDigit = 5}
                if(n == 4){fourthDigit = 5}     
            }
            if (shorterstrings2[n].trim().length == 6 && !(shorterstrings2[n].includes(letterB) && shorterstrings2[n].includes(letterA)))
            {        
                if(n == 1){firstDigit = 6}
                if(n == 2){secondDigit = 6} 
                if(n == 3){thirdDigit = 6}
                if(n == 4){fourthDigit = 6}     
            }
            if (shorterstrings2[n].trim().length == 3)
            {        
                if(n == 1){firstDigit = 7}
                if(n == 2){secondDigit = 7} 
                if(n == 3){thirdDigit = 7}
                if(n == 4){fourthDigit = 7}     
            }
            if (shorterstrings2[n].trim().length == 7)
            {        
                if(n == 1){firstDigit = 8}
                if(n == 2){secondDigit = 8} 
                if(n == 3){thirdDigit = 8}
                if(n == 4){fourthDigit = 8}     
            }
            if (shorterstrings2[n].trim().length == 6 && shorterstrings2[n].includes(letterE) && shorterstrings2[n].includes(letterB)  && shorterstrings2[n].includes(letterF) && shorterstrings2[n].includes(letterA))
            {        
                if(n == 1){firstDigit = 9}
                if(n == 2){secondDigit = 9} 
                if(n == 3){thirdDigit = 9}
                if(n == 4){fourthDigit = 9}     
            }    
        }
        console.log(firstDigit * 1000 + secondDigit * 100 + thirdDigit * 10 + fourthDigit)
        counter2 += firstDigit * 1000 + secondDigit * 100 + thirdDigit * 10 + fourthDigit;
    }
